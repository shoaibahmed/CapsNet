...
# Directories
parser.add_option("--checkpointDir", action="store", type="string", dest="checkpointDir", default="./checkpoints-no-RBA", help="Path for storing checkpoints")
parser.add_option("--logDir", action="store", type="string", dest="logDir", default="./logs-no-RBA", help="Path for storing logs")
parser.add_option("--modelName", action="store", type="string", dest="modelName", default="capsnet-no-RBA", help="Name of the model to be stored")
...
def initCapsNet(inputPlaceholder):
	...
	caps2Predicted_reduced = tf.reduce_sum(caps2Predicted, axis=1, keep_dims=True, name="caps2Predicted_reduced")
	caps2Output = squashFunction(caps2Predicted_reduced, axis=-2, name="caps2Output")

	print ("Caps2 output shape: %s" % str(caps2Output.get_shape()))
	return caps2Output
